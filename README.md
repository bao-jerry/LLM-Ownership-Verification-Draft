...
**Important:** The practical problem framing of this research is currently undergoing a major change. For the smoothest repo onboarding experience, it is important to review the [# Understanding This Repo](#understanding-this-repo) section's clarification about reframing before reading this repo's files.
...
[PROOFS_AND_DERIVATIONS.pdf](PROOFS_AND_DERIVATIONS.pdf) contains all core proofs, derivations, and theoretical results involved in this research project. However, it is *also* the archived version of my first paper draft, one with a significantly different practical problem framing than my current one despite sharing the same theoretical core. This is why you'll notice that it is titled *Yes, That’s Mine: Asymptotically Foolproof LLM Ownership Identification Against Hidden Adversarial Decoding Parameter Perturbations* even though this repo is titled *Provable LLM Tamper Detection: Catching Model Provider Tampering of Open-Weights LLMs*. The reason for my reframing is that my theoretical results map to *tamper detection* much more organically than to model ownership identification. I am currently rewriting the paper with the tamper detection reframing while preserving all the original core theoretical machinery. In the meantime, please refer to [PROOFS_AND_DERIVATIONS.pdf](PROOFS_AND_DERIVATIONS.pdf) if you would like to see the main proofs, theorems, and mathematical arguments that I will be carrying over, while keeping in mind that the new version of the paper that I am now working on will carry a substantially different practical framing.
