...
For comparability with the model re-routing experiment, the same 14B canary prompts are used:
...
Early qualitative read: `4bit` appears substantially shifted from both `bf16` and `8bit` on the model re-routing 14B canaries, especially prompt 4 and prompt 5. This should make 4-bit quantization easy to detect in the theoretical-limit heatmap check.
